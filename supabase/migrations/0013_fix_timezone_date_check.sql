-- Bug real encontrado en producción: create_reservation_atomic
-- rechazaba reservas para "hoy" con INVALID_DATES cada vez que la
-- fecha UTC del servidor ya había avanzado al día siguiente respecto
-- a la fecha local de quien reservaba (ej. después de las 7pm en
-- Perú, UTC-5, ya es el día siguiente en UTC).
--
-- check_in_date/check_out_date son fechas de calendario puras (sin
-- hora ni zona horaria) a propósito — el proyecto entero evita
-- timestamps con zona horaria para estas dos columnas precisamente
-- para no tener este tipo de problema (ver README). El bug estaba en
-- comparar esa fecha "ingenua" contra current_date, que sí depende de
-- la zona horaria configurada en la base de datos (UTC en Supabase).
--
-- Sin un concepto de "zona horaria del hotel" configurable (que
-- tampoco pediría el MVP), la corrección honesta es dar un día de
-- margen: se rechaza solo lo que sea claramente pasado (ayer o antes
-- en UTC), no "hoy" según el huésped pero "mañana" según el servidor.
